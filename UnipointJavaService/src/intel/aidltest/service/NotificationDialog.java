...
public class NotificationDialog {
	...
	public NotificationDialog(Context context)
	{
		...
		inflateManager = (LayoutInflater) context.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
	...
	public void show(String message,int duration, int mcolor, int mtextsize)
	{
		try{
			...
			if(null == mNotificationDialog){
				mNotificationDialog = (View) inflateManager.inflate(R.layout.unipointnotificationtoast, null);
...
//				lp.y = 0;
				...
				{
					Log.v("DEBUG","Add View got exception, may already added ");
				}
			...
			//It is shown already 
				TextView descriptions  = (TextView)mNotificationDialog.findViewById(R.id.notificationdescription);
			...
			if(Unipoint_ServiceActivity.bDEBUG)
				Log.v("DEBUG","Start to Show the dialog, context :"+message+" textsize : "+ mtextsize+" Color :"+mcolor);
			...
			boolean ret = mUIthreadHandler.postDelayed(mRemoveWindowRunnable, duration);
			if(ret)
			{
				if(Unipoint_ServiceActivity.bDEBUG)
					Log.v("DEBUG","Add delayed callbakc, duration  Successfully"+duration);
				
			}else
			{			
				if(Unipoint_ServiceActivity.bDEBUG)
				Log.v("DEBUG","Add delayed callbakc, duration  Failed"+duration);
