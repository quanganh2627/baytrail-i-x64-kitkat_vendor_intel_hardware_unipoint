...
int StateMachine_process(int uinput_fd, gesture_output gest_out,centroid_output cent_out )
{
	...
	switch(gCurMode)
	{
		case MODE_NORMAL:
		{
			...
			switch(gest_out.type)
			{
				...
				case NONE_GESTURE :
					...
					break;
				case RIGHT_GESTURE :
					fprintf(stdout, "%s, MODE_NORMAL, Daemon Received RIGHT_GESTURE event \n",ctime(&timep));
					event_send_gesture(MASK_RIGHT_GESTURE,cent_out,uinput_fd);
					break;
				case LEFT_GESTURE :
					fprintf(stdout, "%s, MODE_NORMAL, Daemon Received LEFT_GESTURE event \n",ctime(&timep));
					event_send_gesture(MASK_LEFT_GESTURE,cent_out,uinput_fd);
					break;
				case DOWN_GESTURE :
					fprintf(stdout, "%s, MODE_NORMAL, Daemon Received DOWN_GESTURE event \n",ctime(&timep));
					event_send_gesture(MASK_DOWN_GESTURE,cent_out,uinput_fd);
					break;
				case UP_GESTURE :
					fprintf(stdout, "%s, MODE_NORMAL, Daemon Received UP_GESTURE event \n",ctime(&timep));
				...
				case POINTER_GESTURE :
					fprintf(stdout, "%s, MODE_NORMAL, Daemon Received POINTER_GESTURE event \n",ctime(&timep));
					event_send_gesture(MASK_POINTER_GESTURE,cent_out,uinput_fd);
					break;
				case TIMEDOUT_GESTURE :
					fprintf(stdout, "%s, MODE_NORMAL, Daemon Received TIMEDOUT_GESTURE event \n",ctime(&timep));
					event_send_gesture(MASK_POINTER_GESTURE,cent_out,uinput_fd);
					break;
				default:
		...
		case MODE_VOLUME:
		{
			...
			switch(gest_out.type)
			{
				...
				case NONE_GESTURE :
					...
					break;
				case RIGHT_GESTURE :
					fprintf(stdout, "%s, MODE_VOLUME,Daemon Received RIGHT_GESTURE event \n",ctime(&timep));
				//	event_send_gesture(MASK_RIGHT_GESTURE,cent_out,uinput_fd);
					break;
				case LEFT_GESTURE :
					fprintf(stdout, "%s, MODE_VOLUME,Daemon Received LEFT_GESTURE event \n",ctime(&timep));
				//	event_send_gesture(MASK_LEFT_GESTURE,cent_out,uinput_fd);
					break;
				case DOWN_GESTURE :
					fprintf(stdout, "%s, MODE_VOLUME,Daemon Received VOLUME_DOWN event \n",ctime(&timep));
					event_send_gesture(MASK_VOLUME_DOWN,cent_out,uinput_fd);
					break;
				case UP_GESTURE :
					fprintf(stdout, "%s, MODE_VOLUME,Daemon Received VOLUME_UP event \n",ctime(&timep));
					event_send_gesture(MASK_VOLUME_UP,cent_out,uinput_fd);
					break;
				case TAP_GESTURE :
					fprintf(stdout, "%s, MODE_VOLUME,Daemon Received TAP_GESTURE event \n",ctime(&timep));
				//	event_send_gesture(MASK_TAP_GESTURE,cent_out,uinput_fd);
								
					break;
				case TAPHOLD_GESTURE :
					fprintf(stdout, "%s, MODE_VOLUME,Daemon Received TAPHOLD_GESTURE event \n",ctime(&timep));
				//	event_send_gesture(MASK_TAPHOLD_GESTURE,cent_out,uinput_fd);
					break;
				case POINTER_GESTURE :
					fprintf(stdout, "%s, MODE_VOLUME,Daemon Received POINTER_GESTURE event \n",ctime(&timep));
				//	event_send_gesture(MASK_POINTER_GESTURE,cent_out,uinput_fd);
					break;
				case TIMEDOUT_GESTURE :
					fprintf(stdout, "%s, MODE_VOLUME,Daemon Received TIMEDOUT_GESTURE event \n",ctime(&timep));
				//	event_send_gesture(MASK_POINTER_GESTURE,cent_out,uinput_fd);
					break;
				default:
		...
		default:
		{
			fprintf(stdout, "%s,Uncrecognized MODE \n",ctime(&timep));
			break;
		}
	}
